...
outdir="scan-$VHOST-$(date '+%Y-%m-%d-%T')"
mkdir -pv "$outdir" && cd "$outdir"
...
echo curl --path-as-is -skg -o "$(date +%s).html" -w "Status: %{http_code}, Length: %{size_download}\n" -H "X-Originally-Forwarded-For: 127.0.0.1, 68.180.194.242" -X GET "$URL" -H "$user_agent"
code=$(curl --path-as-is -skg -o "$(date +%s).html" -w "Status: %{http_code}, Length: %{size_download}\n" -H "X-Originally-Forwarded-For: 127.0.0.1, 68.180.194.242" -X GET "$URL" -H "$user_agent")
print
...
echo -e ${blue}"----------------------"${end}
echo -n "GET : "
echo curl --path-as-is -skg "$URL" -w "Status: %{http_code}, Length: %{size_download}\n" -L -o "$(date +%s).html" -H "$user_agent" -X GET
code=$(curl --path-as-is -skg "$URL" -w "Status: %{http_code}, Length: %{size_download}\n" -L -o "$(date +%s).html" -H "$user_agent" -X GET)
print
echo -n "POST : "
echo curl --path-as-is -skg "$URL" -w "Status: %{http_code}, Length: %{size_download}\n" -L -o "$(date +%s).html" -H "$user_agent" -X POST
code=$(curl --path-as-is -skg "$URL" -w "Status: %{http_code}, Length: %{size_download}\n" -L -o "$(date +%s).html" -H "$user_agent" -X POST)
print
echo -n "HEAD :"
echo curl --path-as-is -skg "$URL" -w "Status: %{http_code}, Length: %{size_download}\n" -L -o "$(date +%s).html" -I -H "$user_agent"
code=$(curl --path-as-is -skg "$URL" -w "Status: %{http_code}, Length: %{size_download}\n" -L -o "$(date +%s).html" -I -H "$user_agent")
print
echo -n "OPTIONS : "
echo curl --path-as-is -skg "$URL" -w "Status: %{http_code}, Length: %{size_download}\n" -L -o "$(date +%s).html" -H "$user_agent" -X OPTIONS
code=$(curl --path-as-is -skg "$URL" -w "Status: %{http_code}, Length: %{size_download}\n" -L -o "$(date +%s).html" -H "$user_agent" -X OPTIONS)
print
echo -n "PUT : "
echo curl --path-as-is -skg "$URL" -w "Status: %{http_code}, Length: %{size_download}\n" -L -o "$(date +%s).html" -H "$user_agent" -X PUT
code=$(curl --path-as-is -skg "$URL" -w "Status: %{http_code}, Length: %{size_download}\n" -L -o "$(date +%s).html" -H "$user_agent" -X PUT)
print
echo -n "TRACE : "
echo curl --path-as-is -skg "$URL" -w "Status: %{http_code}, Length: %{size_download}\n" -L -o "$(date +%s).html" -H "$user_agent" -X TRACE
code=$(curl --path-as-is -skg "$URL" -w "Status: %{http_code}, Length: %{size_download}\n" -L -o "$(date +%s).html" -H "$user_agent" -X TRACE)
print
echo  -n "PATCH : "
echo curl --path-as-is -skg "$URL" -w "Status: %{http_code}, Length: %{size_download}\n" -L -o "$(date +%s).html" -H "$user_agent" -X PATCH
code=$(curl --path-as-is -skg "$URL" -w "Status: %{http_code}, Length: %{size_download}\n" -L -o "$(date +%s).html" -H "$user_agent" -X PATCH)
print
echo  -n "TRACK : "
echo curl --path-as-is -skg "$URL" -w "Status: %{http_code}, Length: %{size_download}\n" -L -o "$(date +%s).html" -H "$user_agent" -X TRACK
code=$(curl --path-as-is -skg "$URL" -w "Status: %{http_code}, Length: %{size_download}\n" -L -o "$(date +%s).html" -H "$user_agent" -X TRACK)
print
echo  -n "CONNECT : "
echo curl --path-as-is -skg "$URL" -w "Status: %{http_code}, Length: %{size_download}\n" -L -o "$(date +%s).html" -H "$user_agent" -X CONNECT
code=$(curl --path-as-is -skg "$URL" -w "Status: %{http_code}, Length: %{size_download}\n" -L -o "$(date +%s).html" -H "$user_agent" -X CONNECT)
print
echo  -n "UPDATE : "
echo curl --path-as-is -skg "$URL" -w "Status: %{http_code}, Length: %{size_download}\n" -L -o "$(date +%s).html" -H "$user_agent" -X UPDATE
code=$(curl --path-as-is -skg "$URL" -w "Status: %{http_code}, Length: %{size_download}\n" -L -o "$(date +%s).html" -H "$user_agent" -X UPDATE)
print
echo  -n "LOCK : "
echo curl --path-as-is -skg "$URL" -w "Status: %{http_code}, Length: %{size_download}\n" -L -o "$(date +%s).html" -H "$user_agent" -X LOCK
code=$(curl --path-as-is -skg "$URL" -w "Status: %{http_code}, Length: %{size_download}\n" -L -o "$(date +%s).html" -H "$user_agent" -X LOCK)
print


echo -e ${blue}"-----------------------------"${end}
echo -e ${cyan}"[+] URL Tricks Bypass suffif "${end}
echo -e ${blue}"-----------------------------"${end}
